...
const REVERTS: Record<string, MappedTxError> = {
  ...
  NothingToClaim: { tone: "warn", message: "There is nothing to claim for your wallet on this market." },
  BelowMinEntry: { tone: "error", message: "Amount is below the market's minimum entry." },
  AboveMaxEntry: { tone: "error", message: "Amount exceeds your remaining entry limit for this side." },
  ...
  "sequencer down": { tone: "error", message: "Oracle unavailable: the L2 sequencer uptime feed reports downtime." },
  ...
  "asset not configured": { tone: "error", message: "Oracle asset is not configured in the registry." },
  // ERC-20
  "ERC20: transfer amount exceeds balance": {
    tone: "error",
    message: "Insufficient balance — mint or acquire more USDG first.",
  },
  "ERC20: insufficient allowance": { tone: "error", message: "USDG approval is required before entering." },
};
...
export function mapTxError(err: unknown): MappedTxError {
  ...
  if (haystack.toLowerCase().includes("insufficient funds") || haystack.toLowerCase().includes("intrinsic gas")) {
  ...
  if (haystack.toLowerCase().includes("already pending")) {
    return { tone: "warn", message: "A transaction is already pending — wait for it to confirm first." };
  }
  if (haystack.toLowerCase().includes("network") || haystack.toLowerCase().includes("rpc")) {
    return { tone: "error", message: "RPC error — check that the local chain / RPC endpoint is running." };
  }
