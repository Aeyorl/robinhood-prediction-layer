...
Responses are `PENDING` until the worker projects the entry, or `CONFIRMED` when it is already indexed. Both represent `SESSION_CORRELATED`, never trustless router attribution. Advisory transaction locks serialize API/worker updates for an entry and prevent concurrent reuse of a swap. Quotes and attribution require the database; asset discovery degrades explicitly to configured known tokens when the database is unavailable.
